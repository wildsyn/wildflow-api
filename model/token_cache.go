...
return out`
	raw, err := common.RDB.Eval(ctx, script,
		[]string{tokenRevokedFenceKeyOf(hmacKey), tokenCacheKeyOf(hmacKey), getTokenCacheFenceKey(key)}).Result()
	if err != nil {
		return nil, err
	}
...
return 1`
	...
	return common.RDB.Eval(context.Background(), script, []string{
		getTokenCacheKey(token.Key), getTokenCacheFenceKey(token.Key), tokenRevokedFenceKeyOf(common.GenerateHMAC(token.Key)),
	},
...
func initTokenCacheFromDatabase(id int, key string) error {
	var current Token
	if err := DB.Where("id = ? AND "+commonKeyCol+" = ?", id, key).First(&current).Error; err != nil {
		return err
	}
	_, err := cacheInitToken(current)
	return err
}
