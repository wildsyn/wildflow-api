...
func TestGetWildFlowCatalogMergesOnlyCanonicalRuntimeAvailability(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":[
			...
			{"id":"Qwen/Qwen3.8-27B-FP8","model_version_ref":"Qwen/Qwen3.8-27B-FP8","callable":true},
	...
	assert.Equal(t, []string{"VoxCPM2", "FLUX.2 [klein] 4B", "Qwen/Qwen3.8-27B-FP8", WildFlowModelExamDualASR}, []string{
	...
	assert.Equal(t, "chat", catalog[2].Kind)
	assert.Equal(t, "Qwen/Qwen3.8-27B-FP8", catalog[2].ModelVersionRef)
