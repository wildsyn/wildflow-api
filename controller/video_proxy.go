...
func VideoProxy(c *gin.Context) {
	...
	if err := proxyTaskMedia(c, task, descriptor); err != nil {
		...
		if !c.Writer.Written() && errors.As(err, &proxyErr) && proxyErr.code == "artifact_request_rejected" {
			// The existing video endpoint exposes blocked fetches as a generic
			// 403. Keep that contract while sharing the new artifact proxy.
