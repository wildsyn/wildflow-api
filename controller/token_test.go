...
func verifyAPITokenAudit(t *testing.T) {
	...
	for index, tc := range []struct {
		...
		{name: "invalid create", expectedStatus: 400, method: "POST", path: "/", body: `{"name":"attempt","remain_quota":-1}`, action: "token.create", params: `{"name":"attempt"}`},
		{name: "malformed body", expectedStatus: 400, method: "POST", path: "/", body: `{"key":"raw-body-secret"`, action: "token.create", params: `{}`},
		{name: "validation error exceeds audit buffer", expectedStatus: 400, method: "POST", path: "/", body: `{"remain_quota":` + strings.Repeat("9", 64*1024) + `}`, action: "token.create", params: `{}`},
		...
		{name: "foreign update", expectedStatus: 404, method: "PUT", path: "/", body: `{"id":$other,"name":"forged-name","unlimited_quota":true}`, action: "token.update", params: `{"id":$other}`},
		...
		{name: "expired enable", expectedStatus: 400, method: "PUT", path: "/?status_only=true", body: `{"id":$id,"status":1}`, action: "token.status_update", params: `{"id":$id,"name":"owned"}`, initialStatus: common.TokenStatusExpired},
		{name: "delete", method: "DELETE", path: "/$id", action: "token.delete", success: true, params: `{"id":$id,"name":"owned"}`},
		{name: "foreign delete", expectedStatus: 404, method: "DELETE", path: "/$other", action: "token.delete", params: `{"id":$other}`},
		{name: "missing delete", expectedStatus: 404, method: "DELETE", path: "/999999", action: "token.delete", params: `{"id":999999}`},
		...
		{name: "foreign key view", expectedStatus: 404, method: "POST", path: "/$other/key", action: "token.key_view", params: `{"id":$other}`},
		...
		{name: "empty batch keys", expectedStatus: 400, method: "POST", path: "/batch/keys", body: `{"ids":[]}`, action: "token.key_view_batch", params: `{"requested_ids":[],"total":0}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "successful response exceeds audit buffer" {
